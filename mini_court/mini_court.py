...
class MiniCourt():
    ...
    def set_canvas_background_box_position(self,frame):
        ...
        self.end_x = frame.shape[1] - self.buffer # This will count 50 points backward from the end of the frame
    ...
    def draw_background_rectangle(self,frame): # This will draw on single frame
        #Create an 8bits dummy image
        shapes = np.zeros_like(frame,np.uint8)
        # Draw the a while rectangle
        cv2.rectangle(shapes, (self.start_x, self.start_y), (self.end_x, self.end_y), (255, 255, 255), cv2.FILLED)
        out = frame.copy()
        alpha=0.5
        # Convert image matrix to boolean
        mask = shapes.astype(bool)

        out[mask] = cv2.addWeighted(frame, alpha, shapes, 1 - alpha, 0)[mask]
        
        return out
        # The cv2.addWeighted() function in OpenCV is used to blend two images together by calculating the weighted sum of the images. 
        # It is commonly used to create transparent overlays, image blending, or combining multiple images for visual effects.

        # Syntax
        # dst = cv2.addWeighted(src1, alpha, src2, beta, gamma)

        # src1: The first input image (array-like object).
        # alpha: The weight of the first image.
        # src2: The second input image (array-like object).
        # beta: The weight of the second image.
        # gamma: A scalar added to the weighted sum, often used for brightness adjustment.


    def draw_mini_court(self,frames): # This will draw on multiple frames
        output_frames = []
        for frame in frames:
    ...
    def get_mini_court_coordinates(self,
        ...
        # Conver pixel distance to meters using convert_pixel_distance_to_meters (cross multiply)
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self,player_boxes, ball_boxes, original_court_key_points ):
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ...
            for player_id, bbox in player_bbox.items():
                ...
                closest_key_point_index = get_closest_keypoint_index(foot_position,original_court_key_points, [0,2,12,13]) # 0, 2, 12, and 13 correspond to the extremes and the middle lines
                ...
                # Get Player height in pixels (maximum height accross 50 pixels)
                ...
                if closest_player_id_to_ball == player_id:
                    # Get The closest keypoint in pixels to the ball
